	...
	<body style="background-color: #f4e500">
		...
		<form id="form1" class="row g-3" style="padding: 15px ;">
			...
			<div class="col-3">
				...
				<input type="text" class="form-control" value=<%= orderDto.getOtype() %> readonly>
			</div>
			<div class="col-3">
				<label for="inputAddress" class="form-label">繳費年期</label> 
				<input type="text" class="form-control" value=<%= orderDto.getOpayType() %> readonly>
			</div>
			<div class="col-3">
			...
			<div class="col-3">
				...
				<input type="date" class="form-control" value=<%= orderDto.getOdate() %> readonly>
			</div>
			<div class="col-6">
				<label for="inputAddress2" class="form-label">負責業務</label> 
				<input type="text" class="form-control" value=<%= orderDto.getOcharge() %> readonly>
			</div>
			<div class="col-md-6">
				<label for="inputCity" class="form-label">負責業務聯絡方式</label> 
				<input type="text" class="form-control" value=<%= orderDto.getOcontact() %> readonly>
			</div>
			<p/>
			<div class="col-12">
				<div class="input-group">
					<span class="input-group-text">備註</span>
					<textarea class="form-control" aria-label="With textarea" placeholder="限制100字以內" readonly><%= orderDto.getOcomment() %></textarea>
				</div>
			</div>
			<div class="col-12">
				<a href="/InsuranceSystem/order" class="button-secondary pure-button" style="display: ${userCert.role == 'EMPLOYEE' || userCert.role == 'ADMIN' ? 'inline-block' : 'none'};">返回上一頁</a>
				<a href="/InsuranceSystem/user/info?cardNumber=${ userCert.cardNumber }" class="button-secondary pure-button" style="display: ${userCert.role == 'CUSTOMER' ? 'inline-block' : 'none'};">返回上一頁</a>
