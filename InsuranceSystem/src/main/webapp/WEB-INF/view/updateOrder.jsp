...
<html>
	...
	<body style="background-color: #f4e500">
		<form style="padding: 15px" class="pure-form" method="post" action="/InsuranceSystem/order/update">
			<fieldset>
				<legend>更改保單負責人員資料</legend>
				保單序號:<input type="text" name="orderId" value="<%= orderDto.getOid() %>" readonly><p />
				...
				繳費狀態:<input type="radio" name="orderStatus" value="true" <%= orderDto.getOstatus() ?"checked":"" %>>已繳完
						<input type="radio" name="orderStatus" value="false" <%= orderDto.getOstatus() ?"":"checked" %>>未繳完<p />
				<textarea class="form-control" name="comment"><%=orderDto.getOcomment() %></textarea>
